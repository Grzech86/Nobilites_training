package Nobilities.ex32;

import java.util.*;

//Zadanie 32: Calculate an average grade of students per city
public class ex32 {
...
//name, age, city, grade, faculties
    ...
    public static void main(String[] args) {
        ...
        Map<String, List<Student>> cityMap = new HashMap<>();
        ...
        Map<String, Double> cityMapOfAverages = new HashMap<>();

        final var grouped = cityMap.entrySet();
        for(final var g : grouped) {
            final var listOfStudents = g.getValue();
            final var city = g.getKey();

            double gradeSum = 0.0;
            for (Student current : listOfStudents) {
                gradeSum += current.grade;
            }

            gradeSum /= listOfStudents.size();

            cityMapOfAverages.put(city, gradeSum);
        }



        System.out.println(cityMapOfAverages);
